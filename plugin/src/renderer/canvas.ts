...
export function drawGroup(
  ...
  // At very low zoom, make groups more prominent
  const isLowZoom = vt.zoom < ZOOM_LOD_COMPACT;
  ctx.fillStyle = isLowZoom ? c + '15' : c + '08';
  ctx.strokeStyle = isLowZoom ? c + '30' : c + '18';
  ctx.lineWidth = isLowZoom ? 1.5 : 1;
  ...
  // Show node count in label at low zoom
  const label = (isLowZoom && nodeCount !== undefined)
    ? `${group.label} (${nodeCount})`
    : group.label;
  ctx.fillText(label, x + 10 * vt.zoom, y - 4 * vt.zoom);
}

/** Draw a collapsed group as a summary pill */
export function drawCollapsedGroup(
  ctx: CanvasRenderingContext2D,
  group: GroupDef,
  vt: ViewTransform,
  theme: ThemeColors,
  ...
  // Draw at group center, as a rounded pill
  const pillW = 180 * vt.zoom;
  const pillH = 50 * vt.zoom;
  const cx = (group.x + group.w / 2) * vt.zoom + vt.panX;
  const cy = (group.y + group.h / 2) * vt.zoom + vt.panY;
  const px = cx - pillW / 2;
  const py = cy - pillH / 2;
  const r = pillH / 2;

  // Background fill
  ctx.fillStyle = c + (isHovered ? '40' : '25');
  roundRect(ctx, px, py, pillW, pillH, r);
  ctx.fill();

  // Border
  ctx.strokeStyle = c + (isHovered ? 'aa' : '60');
  ...
  ctx.fillText(group.label, cx, cy - fs * 0.35);
  ...
  // Expand icon (small + sign)
  const iconSize = Math.max(6, 8 * vt.zoom);
  const iconX = px + pillW - iconSize * 2.5;
  const iconY = cy;
...
// Schematic mode level-of-detail based on zoom level

/** Below this zoom, nodes render as compact colored pills (no pins, no body) */
const ZOOM_LOD_COMPACT = 0.25;
/** Below this zoom, pin labels are hidden (circles still visible) */
const ZOOM_LOD_NO_PIN_LABELS = 0.4;
/** Below this zoom, pin circles are hidden entirely */
const ZOOM_LOD_NO_PINS = 0.3;
/** Below this zoom, badges are hidden */
const ZOOM_LOD_NO_BADGES = 0.35;

/** Draw a single node (with semantic zoom level-of-detail) */
export function drawNode(
  ...
  // ── LOD: Compact mode — just a colored pill with title ──
  if (vt.zoom < ZOOM_LOD_COMPACT) {
    const compactH = hh + 4 * vt.zoom;
    ctx.fillStyle = cat.dark;
    roundRect(ctx, x, y, w, compactH, r);
    ctx.fill();
  ...
  // ── Full or medium detail rendering ──
  ...
  // 7. Connection count badge (skip at low zoom)
  if (vt.zoom >= ZOOM_LOD_NO_BADGES) {
  ...
  // 8-9. Pins (skip entirely at very low zoom, skip labels at medium zoom)
  const showPins = vt.zoom >= ZOOM_LOD_NO_PINS;
  const showPinLabels = vt.zoom >= ZOOM_LOD_NO_PIN_LABELS;
...
export function renderFrameOrganic(
  ...
  // Wires (drawn first, behind nodes; skip wires to hidden nodes)
  for (let i = 0; i < data.wires.length; i++) {
    const wire = data.wires[i];
    const wireEnds = getWireNodeIds(wire);
    if (hiddenNodeIds && (hiddenNodeIds.has(wireEnds.from) || hiddenNodeIds.has(wireEnds.to))) continue;
...
export function renderFrameFull(
  ...
  // Count nodes per group for low-zoom labels
  const groupNodeCounts = new Map<string, number>();
  for (const group of data.groups) {
    let count = 0;
    for (const node of data.nodes) {
      if (!isNodeVisible(node, data.categories)) continue;
      const nw = (node as any).w ?? NODE_W;
      const nh = (node as any).h ?? 60;
      // Check if node is within group bounds
      if (node.x >= group.x && node.x + nw <= group.x + group.w &&
          node.y >= group.y && node.y + nh <= group.y + group.h) {
        count++;
      }
    }
    groupNodeCounts.set(group.label, count);
  }

  // Groups (skip collapsed ones — they get drawn as pills separately)
  for (const group of data.groups) {
    if (group.collapsed) continue;
    drawGroup(ctx, group, vt, theme, data.categories, groupNodeCounts.get(group.label));
  }
  ...
  // Wires (skip wires connected to hidden/collapsed nodes)
  for (let i = 0; i < data.wires.length; i++) {
    const wire = data.wires[i];
    const ends = getWireNodeIds(wire);
    if (hiddenNodeIds && (hiddenNodeIds.has(ends.from) || hiddenNodeIds.has(ends.to))) continue;
  ...
  // Draw collapsed group pills
  for (const group of data.groups) {
    if (!group.collapsed) continue;
    const count = group.nodeIds?.length ?? 0;
    drawCollapsedGroup(ctx, group, vt, theme, count, false);
  }
}
